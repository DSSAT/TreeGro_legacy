...
C=======================================================================
     ...
     & TRTN,CHEXTR,ISENS, BEDHT, BEDWD, DripSpc, DripOfset)  
      ...
      INTEGER TRTN,ISENS  !NFORC,NDOF,PMTYPE,

      REAL    BEDHT, BEDWD, DripSpc, DripOfset !, PLTFOR
...
C-----------------------------------------------------------------------
     ...
     &       FLDD,SFDRN,FLST,SLTX,SLDP,SLNO,BEDHT, BEDWD,PMALB
      IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,ERRNUM,FILEIO,LINIO)
      WRITE (LUNIO,60,IOSTAT=ERRNUM) XCRD,YCRD,ELEV,AREA,SLEN,FLWR,SLAS
     &            , FldHist, FHDur !, PMALB
      IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,ERRNUM,FILEIO,LINIO)
!      WRITE (LUNIO,65,IOSTAT=ERRNUM)BEDHT, BEDWD, PMALB
!      IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,ERRNUM,FILEIO,LINIO)
C-----------------------------------------------------------------------
C
C-----------------------------------------------------------------------
...
! 09/02/2008 CHP added BEDWD, BEDHT (raised bed width and height), 
!                removed pineapple-specific parameters
      LINIO = LINIO + 1
...
!     &          NFORC,PLTFOR,NDOF,PMTYPE
    ...
    ! &          BEDWD, BEDHT
     ...
     &          SDAGE,ATEMP,PLPH,SPRLAP !, BEDWD, BEDHT
         ENDIF
      ENDIF
      IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,ERRNUM,FILEIO,LINIO)
...
C-----------------------------------------------------------------------
     ...
     &       IAMEX,AIRAMX, DripSpc, DripOfset
      IF (ERRNUM .NE. 0) CALL ERROR (ERRKEY,ERRNUM,FILEIO,LINIO)
      IF (NIRR > 0) THEN
        DO I = 1,NIRR
          LINIO = LINIO + 1
...
!chp   59 FORMAT (3X,A8,1X,A8,1X,F5.1,1X,F5.0,1X,A5,2(1X,F5.0),
   59 FORMAT (3X,A8,1X,A8,1X,F5.1,1X,F5.0,1X,A5,1X,F5.0,1X,F5.1,
     &        2(1X,A5),1X,F5.0,1X,A10,2(1X,F5.0), F6.2)
...
C-Y2K &        2(1X,F5.0),3(1X,F5.1),I6,F6.1,2I6)
   70 FORMAT (3X,I7,1X,I7,2(1X,F5.1),2(5X,A1),2(1X,F5.0),1X,F5.1,
     &        2(1X,F5.0),3(1X,F5.1),2F6.0)
   71 FORMAT (3X,I7,1X,I7,2(1X,F5.1),2(5X,A1),2(1X,F5.0),1X,F5.1,
     &        I6,1X,F5.0,3(1X,F5.1),2F6.0)
   75 FORMAT (2X,1X,F5.3,3(1X,F5.0),2(1X,A5),1X,F5.1,2F6.0)
