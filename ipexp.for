...
C=======================================================================
     ...
     &           BEDHT, BEDWD, DripSpc, DripOfset)
      ...
      INTEGER YR,ROTN !NFORC,NDOF,PMTYPE,
      INTEGER TRTNUM, ROTNUM

      REAL    FLAG,EXP,TRT, BEDHT, BEDWD, DripSpc, DripOfset  !,PLTFOR
...
C-GH        TRTN   = 1
         ...
         ENDIF
       ELSEIF (INDEX ('PQ',RNMODE) .GT. 0) THEN
         !READ (TRNARG(1:6),'(I6)') TRTN
         !READ (ROTNARG(1:6),'(I6)') ROTN
         TRTN = TRTNUM
         ROTN = ROTNUM
         I = 999
       ELSEIF (INDEX ('NPQGSFBECT',RNMODE) .GT. 0) THEN
!         READ (TRNARG(1:6),'(I6)') TRTN
         TRTN = TRTNUM
         I = 999
...
C     IF (I .LT. TRTN) GO TO 50
      ...
      !&    (INDEX('PQ',RNMODE) .GT. 0 .AND. ! JZW modified:ignore ROTNO if RNMODE=p 
     &    (INDEX('Q',RNMODE) .GT. 0 .AND. 
     &                     (TRTN .NE. TRTNO .OR. ROTN .NE. ROTNO)) .OR. 
     &    (INDEX('AI',RNMODE) .GT. 0 .AND. I .LT. TRTN))
     &    GO TO 50
      ! JZW changed in Feb, 2012 
       IF ((INDEX('P',RNMODE) .GT. 0) .AND. (TRTN .NE. TRTNO)) go to 50
...
C-----------------------------------------------------------------------
     ...
     &     YRPLT,SPRLAP,IPLTI) !NFORC,PLTFOR,NDOF,PMTYPE,
      !&     YRPLT,SPRLAP,IPLTI, BEDHT, BEDWD) !NFORC,PLTFOR,NDOF,PMTYPE,
...
!     Skip soils field and soils input for sequence mode
      IF (INDEX('FPQ',RNMODE) .LE. 0 .OR. RUN == 1) THEN
...
C=======================================================================
    ...
    !&     YRPLT,SPRLAP,IPLTI, BEDHT, BEDWD) !NFORC,PLTFOR,NDOF,PMTYPE,
     &     YRPLT,SPRLAP,IPLTI) !NFORC,PLTFOR,NDOF,PMTYPE,
      ...
      REAL      PLPH,SPRLAP !, BEDHT, BEDWD !,PLTFOR
...
!     CHP 9/2/2008 Add bed width input; remove NFORC,PLTFOR,NDOF,PMTYPE
     &      PLPH,SPRLAP !, BEDWD, BEDHT
            IF (ERRNUM .NE. 0) CALL ERROR(ERRKEY,ERRNUM,FILEX,LINEXP)
            CALL Y2K_DOY(YRPLT)
            CALL Y2K_DOY(IEMRG)
...
!60   FORMAT (I3,I5,1X,I5,2(1X,F5.0),2(5X,A1),8(1X,F5.0),I6,F6.0,2I6)
 60   FORMAT (I3,I5,1X,I5,2(1X,F5.0),2(5X,A1),10(1X,F5.0))
...
C
 ...
 70     CALL IGNORE (LUNEXP,LINEXP,ISECT,CHARTEST)
    ...
    ! Write(msg(1),'("Plastic mulch cover abedo =",F7.2)') PMALB 
    !  call info(1,errkey,msg)
...
! 80   FORMAT (I3,2(F15.0,1X),F9.0,1X,F17.0,3(1X,F5.0))
 80   FORMAT (I3,2(F15.0,1X),F9.0,1X,F17.0,3(1X,F5.0),1X,A5,I6,F6.0)
 90   FORMAT (I3,2(F5.0,1X),F6.0)
