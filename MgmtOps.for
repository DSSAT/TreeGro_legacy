...
C-----------------------------------------------------------------------
      ...
      REAL HPC(NHARD), HBPC(NHARD), HARVFRAC(2)
...
!     Variables added for flooded conditions
      ...
      INTEGER HDLAY, HLATE, HSTG(NHARD)
...
C-----------------------------------------------------------------------
      ...
      ELSEIF (INDEX('PQ',RNMODE) .GT. 0) then !????JZW question: what to do for RNMOD=p
...
!         Get unit number for daily and seasonal irrigation output files
     ...
     &      IOSTAT = ERRNUM)
        ...
        IF (INDEX('FPQN',RNMODE) <= 0 .OR. RUN == 1) THEN
...
!       Harvest
     ...
     &            "Harvest        ", SumDat % HWAH, " kg/ha"
          IF (INDEX('FPQ',RNMODE) > 0 .AND. CROP /= 'FA') THEN
