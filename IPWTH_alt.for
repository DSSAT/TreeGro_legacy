...
C=======================================================================
     ...
     &    CCO2, FILEW, FILEWW, MeanPrecip, MEWTH, PAR,    !Output
     &    PATHWT, RAIN, REFHT, RHUM, RRC, RSEED1, SRAD,   !Output
...
!     Optional regional rainfall coefficient and mean annual precipitation
!       for J.T. Ritchie's Salus runoff routine
!     Need citation for coefficients map
      REAL RRC, MeanPrecip
...
!-----------------------------------------------------------------------
      IF (INDEX('FPQ',RNMODE) <= 0 .OR. RUN. EQ. 1) THEN
...
!     Don't re-initialize for sequence and seasonal runs
      IF (INDEX('FPQ',RNMODE) > 0 .AND. RUN > 1) RETURN
      !JZW for RNMOD=p, do not return???
...
!       Read corresponding line of data 
     ...
     &              //  ' will be used.'
              ...
              CASE('RRC')
!               Regional runoff coefficient and mean annual rainfall 
!               used for Salus surface runoff routine (in RNOFF, MEINF = "L")
                READ(LINE(C1:C2),*,IOSTAT=ERR) RRC
                IF (ERR .NE. 0) RRC = -99.0

              CASE('MNRN')
!               Mean annual rainfall used for Salus surface runoff 
!               routine (in RNOFF, MEINF = "L")
                READ(LINE(C1:C2),*,IOSTAT=ERR) MeanPrecip
                IF (ERR .NE. 0) MeanPrecip = -99.0
            END SELECT
          ENDIF
        ENDDO
...
!      END SELECT
      ...
      IF (INDEX('FPQ',CONTROL%RNMODE) > 0) THEN
        I = LEN_TRIM(FILEWW)
