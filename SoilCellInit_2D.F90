...
!-----------------|--------------|----------------------------|-------------|-------------|
!  Bed  | BH < 2  |    CASE 1    |     CASE 2  |    CASE 1    |    CASE 3   |    CASE 1   |
! Height|---------|--------------|----------------------------|-------------|-------------|
!  (cm) | BH > 2  |    CASE 1    |     CASE 4  |    CASE 5    |    CASE 3   |    CASE 1   |
!------------------------------------------------------------------------------------------
...
!    Type (DripIrrType) DripIrrig
    ...
    REAL, DIMENSION(MaxCols) :: ColFrac, BedFrac
...
!   Read plastic mulch albedo from FIELDS section
    ...
    IF (FOUND /= 0)  THEN
      READ(LUNIO,'(80X,2F6.0, F6.2)',IOSTAT=ERR)BEDWD, BEDHT, PMALB
...
!---------------------------------------------------------------------------
    IF (BEDWD < 2.) THEN ! BH<2 or BH >2
      BedCase = 1  !Flat surface, no plastic mulch cover
    ...
    ELSEIF (ROWSPC_CM - BEDWD < 2.) THEN ! BH<2 or BH>2
      IF (PMALB > 1.E-2) THEN
        BedCase = 3  !Flat surface, full plastic mulch cover (not handled yet)
        PMCover   = .TRUE.
        BEDWD = ROWSPC_CM
        BEDHT = 0.
        RaisedBed = .FALSE.  
...
!     No plastic mulch - CASE 1 !JZW BedCase 1,2,3 Flat bed
    ...
    ! JZW add the following for Citrus
    if (CONTROL % CROP .EQ.'CT')N_Fur_Cols=nint((ROWSPC_CM-BEDWD)/2./30.) 
...
!     van Genuchten parameters fit from known soil properties based on RETC code
        CALL RETC_VG(SoilProp_Furrow%TEXTURE(M), SoilProp_Furrow%SWCN(M),          & !Input
           ...
           SoilProp_Furrow%WCR(M), Furrow_Hb, Furrow_lambda,                  & !Input 
...
!       Use furrow layer depths below bed
      ...
      ! JZW for non bed system, goes to the following statement, But we should not doing so.
...
!---------------- Output Cell Structures ---------------------------------
    ...
    Write(msg(1),'(" Column ->",25I5)') (Col, Col=1, NColsTOT)
    Write(msg(2),'("Wid(cm) ->",25F5.1)') (Cells(1,Col)%Struc%width,Col = 1, NColsTOT)
    Write(msg(3),'("     Thick")') 
