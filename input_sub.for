...
!                Removed pineapple planting params: NFORC,PLTFOR,NDOF,PMTYPE
...
!      CHARACTER* 18 RUNARG
      ...
      INTEGER       ISENS, TRTNUM, ROTNUM !NFORC,NDOF,PMTYPE,
...
C-----------------------------------------------------------------------
     ...
     &            IDETC,IDETW,IDETN,IDETP,IDETD,IDETL,IDETH,IDETR
          CLOSE (LUNIO,STATUS = 'DELETE') ! delete a file as a CLOSE option
      ENDIF
...
!     Skip soils field and soils input for sequence mode
      IF (INDEX('FQP',RNMODE) .LE. 0 .OR. RUN == 1) THEN
...
C-----------------------------------------------------------------------
     ...
     &            CHEXTR,ISENS, BEDHT, BEDWD, DripSpc, DripOfset)   
     ...
     &           LNSIM,LNCU,LNHAR,LNENV,LNTIL,LNCHE,
     &           LNFLD,LNSA,LNIC,LNPLT,LNIR,LNFER,LNRES,
     &           CHEXTR, MODEL, PATHEX, BEDHT, BEDWD)
