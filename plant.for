...
C=======================================================================
!***************
!Currently, Plant subroutine works for 2D only.
! For run 1D, we should remove the argument CELLS. Using put get(CELLS, ) for 2D case
! For ipexp, we should set BDHT as optional. if 2D case read BDHT in read statement
!****************
...
!       Write message to WARNING.OUT file
      ...
      ENDIF
      IF (INDEX(MODEL,'TRGRO') <= 0 .AND. ISWITCH % MEPHO .EQ. 'L') THEN
        ISWITCH % MEPHO = 'C'
!       Put ISWITCH data where it can be retreived 
!         by other modules as needed.
        CALL PUT(ISWITCH)
...
!       Write message to WARNING.OUT file
      ...
      ENDIF
      IF (INDEX(MODEL,'TRGRO') <= 0 .AND. ISWITCH % MEEVP .EQ. 'Z') THEN
!       Default to Priestly-Taylor potential evapotranspiration
        ISWITCH % MEEVP = 'R'
!       Put ISWITCH data where it can be retreived 
!         by other modules as needed.
...
!-----------------------------------------------------------------------
!     CROPGRO model
      CASE('CRGRO')  ! JZW we need to modify teh CropGro from CropGro_2D???
...
!     -------------------------------------------------
      CASE('TRGRO') !JZW we need 1D TreeGRO here
...
!       TEMP CHP 8/30/2010
!       Put generic root depth in for now.
!       Eventually, this needs to be sent from each plant module.
        CALL PUT('PLANT', 'RTDEP',  20.)
