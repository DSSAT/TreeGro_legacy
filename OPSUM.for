...
!-----------------------------------------------------------------------
      ! JZW update WSTAT. Necessary for RNMOD=pq, because put is called when (YRDOY > LastWeatherDay)
      ! whereas this subroutine call get at SEASINIT
      CALL GET('WEATHER','WSTA',WSTAT) 
...
C-------------------------------------------------------------------
      IF (INDEX('NPQSABCGF',RNMODE) .GT. 0 .OR. NYRS .GT. 1) THEN
          NLINES = RUN - 1
        IF (RUN .EQ. 1) THEN
          CALL CLEAR
        ENDIF
