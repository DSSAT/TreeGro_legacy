...
! INTEGER NBedRows, NBedCols         !number of bed rows, cols
  ...
  TYPE BedDimType
    REAL BEDHT, BEDWD, DigDep, ROWSPC_cm, DripSpc  !cm
...
!   Column width as fraction of total field surface area
    ...
    INTEGER LIMIT_2D
    INTEGER DripCol     !Column recieving drip irrig and fert
...
! TEMP CHP
  type Cell_detail_type
    integer row, col  !cm
    double precision h_in, h_out, v_in, v_out, vdiff, vgrav  !cm2
  end type
  type (Cell_detail_type) Cell_detail
  
  type Cell_Ndetail_type
    integer row, col  !cm
    real NFlux_L, NFlux_R, NFlux_D, NFlux_U
  end type
