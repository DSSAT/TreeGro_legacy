...
C  Revision history
C  . . . . . .  Written
C  06-01-2010 JJW / CHP Modified SoilNi for 2D.
C-----------------------------------------------------------------------
C  Called : SOIL
...
!     Set initial SOM and nitrogen conditions for each soil layer.
     ...
     &    Cell_Type, SOILPROP, SOILPROP_profile, ST,         !Input
...
!             Cell_Ndetail % NHFlux  = Cells(L, J) % Rate % HFlux 
!             JZW This variable or component must be of a derived or structure type [L]
!             Cell_Ndetail % NVFlux  = Cells(L, J) % Rate % VFlux
          Endif
        ENDDO
      ENDDO
