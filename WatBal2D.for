...
!     TYPE (SoilType)   , INTENT(IN) :: SOILPROP, SOILPROP_furrow
      ...
      CHARACTER*78 MSG(30)
      INTEGER DripNum, DripCol, DYNAMIC, FurRow1, FurCol1, i, j, jj
...
!     temp chp
      ...
      REAL DRAIN_2D, DripSpc, DripOfset, HalfRow, HalfFurrow
      REAL RAIN, RUNOFF, ES
      REAL TEP, TES, SRAD_TOT
      REAL TDRAIN, TRUNOF, TSW, TSWINI
      REAL DripStart, DripDur, DripInt, DripRate
      REAL IrrRate, RWUEP1
      ...
      REAL, DIMENSION(MaxCols) :: ColFrac, WINF_col
      REAL, DIMENSION(MaxRows,MaxCols) :: CellArea, ES_mm
      ...
      REAL, ALLOCATABLE :: IrrigSched(:,:)

      Double Precision DRAIN_ts, EOP_ts, ES_avg, ES_day, ES_ts
      Double Precision INF_vol, IRR_ts, IrrVol, Rain_ts, Runoff_ts
      Double Precision TRWU_ts,TRWUP_ts,ES_vf_ts,SW_VOL_tot,INF_vol_dtal
...
!     Drip irrigation
      CALL GET(DripIrrig)
      DripSpc   = DripIrrig % DripSpc
      BedDimension % DripSpc = DripSpc

!     Determine which column receives drip irrigation
      DripOfset = DripIrrig % DripOfset
      DripCol = 1 !default drip emitter located at center of bed
      WidTot = 0.
      DO j = 1, FurCol1-1
        WidTot = WidTot + Width(1,j)
        IF (WidTot > DripOfset) THEN
          DripCol = j
          EXIT
        ENDIF
      ENDDO
      BedDimension % DripCol = j
...
!     Drip irrigation schedule for today
      ...
      DripNum   = DripIrrig % DripNum   !# of irrigs today
      DripRate  = DripIrrig % DripRate  !emitter rate (ml/s)
      ...
      IF (DripNum > 0 .AND. IrrRate > 1.E-6) THEN
        ALLOCATE (IrrigSched(DripNum,2))
        IrrigSched(1,1) = DripStart
        IrrigSched(1,2) = DripStart + DripDur
        DO i = 2, DripNum
          IrrigSched(i,1) = IrrigSched(i-1,2) + DripInt
          IrrigSched(i,2) = IrrigSched(i,1) + DripDur
        ENDDO
      ELSE 
        DripNum = 0
        DripDur = 0
...
!       Need smaller time step during irrigation
        IF (DripNum > 0 .AND. IrrigIndex <= DripNum) THEN
          IF (IRRIG) THEN
!           Currently in irrig cycle.  Check for end of irrig. 
            IF (ABS(StartTime - IrrigSched(IrrigIndex,2)) < 0.5/60.)THEN
!             End of irrig cycle
...
!           Currently in drying cycle.  Check for start of next irrig. 
            IF (ABS(StartTime - IrrigSched(IrrigIndex,1)) < 0.5/60.)THEN
!             Start new irrig cycle
        ...
        IF (RAIN > 1.E-6) THEN  ! check if both irr and rain exist
          IF (DripInt > 1.E-6) THEN
            DeltaT = Time_interval(DripInt, TSN)  !minutes
          ELSE
            DeltaT = TSN
          ENDIF
        ...
        IF (IRRIG) THEN
!         Irrigated time step
          DeltaT = Time_interval(DripDur, TSI)  !minutes
          TimeIncr = MIN(TimeIncr, DeltaT)
          IF (StartTime + TimeIncr/60. > IrrigSched(IrrigIndex,2)) THEN
!           Don't let time step go beyond end of irrigation
            TimeIncr = (IrrigSched(IrrigIndex,2) - StartTime) * 60.
          ENDIF
        ELSEIF (IrrigIndex <= DripNum) THEN
!         Non-irrigated time step and another irrigation coming up today
          IF (StartTime + TimeIncr/60. > IrrigSched(IrrigIndex,1)) 
     &       THEN
!           Don't let time step go beyond start of next irrigation
            TimeIncr = (IrrigSched(IrrigIndex,1) - StartTime) * 60.
          ENDIF
        ENDIF
...
!       BE CAREFUL WITH TIME INCREMENTS < 1 MINUTE!
        IF (DripNum > 0 .AND. IrrigIndex <= DripNum) THEN
          IF (ABS(EndTime - IrrigSched(IrrigIndex,1)) < 0.5/60.) THEN
            EndTime = IrrigSched(IrrigIndex,1)
          ELSEIF (ABS(EndTime - IrrigSched(IrrigIndex,2)) < 0.5/60.)THEN
            EndTime = IrrigSched(IrrigIndex,2)
          ENDIF
        ENDIF
...
!         Irrigation volume is half dripper rate because only half row is being modeled
          IrrVol = (DripRate  / 2.) / DripSpc * TimeIncr * 60.
...
!            cm3[soil]       d       mm       cm[soil]   
            ...
            if ((Cell_detail%row .EQ. FurRow1) .and.
     &                (Cell_detail%col .eq. j) )  then
              INF_vol_dtal = INF_vol
            Endif
          ENDDO
...
!             temp chp
              if (i == cell_detail%row .and. j == cell_detail%col) then
                es_vf_ts = ES_avg * mm_2_vf(i,j)
              endif
            CASE DEFAULT; CYCLE
            END SELECT
          ENDDO
...
!       Output arrays only once per hour
        if (((StartTime < 23) .OR.
     &       (StartTime >= 23 .AND. NextUpdate == 23.)) .AND. 
     &      (StartTime . GE. float(NextUpdate)))    then
    ! &      StartTime > float(NextUpdate))    then
          NextUpdate = NextUpdate + 1
          if (NextUpdate > 23) NextUpdate = 0
          ! NextUpdate is not used
...
! IRRAMT      Irrigation amount for today    mm
! IRRIG       LOGICAL variable. TRUE means irrigation is on, FALS means off 
...
! Rain_ts     Rain water in current time step
...
! StdIrrig    Standard one dimension effective irrigation amount(mm)
...
!=======================================================================
     ...
     &  RUNOFF, WINF_col)                                 !Output
      ...
      REAL, DIMENSION(MaxCols) :: ColFrac, RUNOFF_col, WINF_col
      ...
      IF (RAIN > 1.E-6) THEN
        IF (FurCol1 > NColsTot) THEN
          RUNOFF = RAIN ! CHP does not remember why this statement. Thus we may remove
          RETURN
        ENDIF

!       Rainfall over a row is concentrated in the furrow
        IF (BedDimension % PMCover) then
...
!         No mulch effects on runoff
          ...
          END IF
          RUNOFF = RUNOFF + RUNOFF_col(j) * ColFrac(j)
