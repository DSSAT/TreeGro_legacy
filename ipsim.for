...
!        METMP   = 'E'
 ...
 50      CALL IGNORE(LUNEXP,LINEXP,ISECT,CHARTEST)
     ...
     &            YRSIM,RRSEED1,TITSIM,CRMODEL
         ...
         !   .AND. RUN .EQ. 1)) THEN
            IF ((INDEX('PQ',RNMODE) .LE. 0) .OR. 
     &      (INDEX('PQ',RNMODE) .GT.0) .AND. (RUN .EQ. 1)) THEN
               RSEED1 = RRSEED1
               IF (RSEED1 .LE. 0) THEN
                 RSEED1 = 2150
               ENDIF
...
C
         CALL IGNORE(LUNEXP,LINEXP,ISECT,CHARTEST)
         IF (INDEX('FPQ',RNMODE) < 1 .OR. RUN == 1) THEN
...
!     from first run
      IF (INDEX('FPQ',RNMODE) <= 0 .OR. CONTROL % RUN == 1) THEN
